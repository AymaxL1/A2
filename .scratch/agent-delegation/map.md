...
- ✅ **06** `ea726d8` SystemAgentPort 真实现(PASS=351,47 条断言,唯一碰真进程的套件)。第一技术判断:**必须 posix_spawn 而非 Foundation `Process`** —— `Process` 没有设置子进程组的公开 API,子进程会继承宿主进程组,`kill(-pgid,…)` 会连宿主一起杀,反孤儿铁律根本无从谈起。CR 修 🔴:并发 `terminate` 的 ECHILD 分支沿用初值 0,两次并发取消会让其中一次把真实的 -15 覆盖成 0 → `resolve` 判 `completed`,**一次取消被误报成成功**;修法 `markReaped(status: nil)` + 首写为准。另:`isAlive` 用 `waitid(WNOWAIT)` **故意不收尸** —— 没收尸的僵尸 leader 把 pgid 钉死在内核里,防止 `kill(-pgid,…)` 落到被复用的别人组上,故「先 reclaim 后 terminate」会永久漏掉僵尸。
- ✅ **07** 试驾 CLI `aa-agent` + 启动参数组装 + 端到端(PASS=480)。**两轴独立收敛到同一个 🔴**:Claude 侧的「stdin 显式收尾」在票面、组装器注释、端口注释三处都写了,**代码里一次都没调用 `closeStdin`**。后果是每一次**成功**的 Claude 委托都要空转满静默阈值(默认 120s)、再被自家看门狗判「卡死」发 SIGTERM 才收得了尾;而 05 的 `succeeded` 豁免会把终态兜回 `completed`,于是**门禁、冒烟、退出码全部照样绿**,盘上却留下 `state=completed` + `exit_code=-15/143` + 一行「看门狗判定卡死」的自相矛盾现场。主会话做了**反证**:注掉那一行后同一条端到端从 1s 变 21s、看门狗行出现、`exit_code=-15`,而旧断言无一变红 —— 确认这是「只朝看起来没事的方向失败」的静默缺陷。修法:监工循环拿到终态即 `closeStdin`。
  其余 CR 修:blocked-args 漏掉 clap **短旗标贴写形态**(`-sdanger-full-access` / `-csandbox_mode=…` / `-C/` / `-mNAME`,按 `=` 切出的 name 不在表里就整条穿过去 —— 「等价旁路不堵等于没堵」这句话必须对自己成立);`--mcp-config` / `--settings` 进 blocked 表(`--strict-mcp-config` 的语义正是「只认经 `--mcp-config` 传进来的那份」,漏堵等于把锁装好再把钥匙插门上);`AgentDelegation.codexHome` **去掉默认值**(漏传原本能编译通过,后果是静默不注 `CODEX_HOME`、子进程回落用户真 `~/.codex`,而那目录「用后即脏」);auth.json 副本改走新增的 `writePrivate`(0600,**刻意不给默认实现** —— 默认实现只能退化成 0644,那正是本模块在防的 fail-open);`aa-agent` 补红线 grep(断言组 3f);相对 `--exec` 在入口折成绝对路径(`launch` 是先 chdir 再 exec,相对路径校验过得了、真拉起必 ENOENT);cancel/launch 窄窗口竞态(盘上已 cancelled 却让进程跑到底烧配额)。
  **新增断言组 1k(本票最该留下的东西)**:用**假 agent 重放 spike 当年真跑落盘的事件流**(claude `01-baseline-readonly.stdout.ndjson`、codex `exec1-baseline-readonly-default.stdout.jsonl`)跑通 run 全链路 —— 协议形状是真的、零网络零配额零真 agent。假 claude 吐完事件流后 `cat > /dev/null` 精确复刻「不发 EOF 就不自退」,故 `closeStdin` 一旦再断,时长上界与「无看门狗字样」两条断言当场变红。此前门禁只跑 `--dry-run`,而 dry-run 在建工作区**之前**就退出了 —— 整条主链路是块盲区。
