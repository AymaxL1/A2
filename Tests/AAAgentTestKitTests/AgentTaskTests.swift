// 17 票:从 `AAAgentTestKit.AgentTaskTests` 迁到 swift-testing
//   (迁移口径见 Tests/AAHostTestKitTests/RegistryConformanceTests.swift 头注)。
//
// 任务状态机 + 工作区落盘的纯逻辑测试(agent-delegation 04)。
//
...
//
// 迁移说明:旧套件每个 sub-function 外面都套了一层 `do { … } catch { report.check(false, "…意外抛错 \(error)") }`。
//   这里**原样保留**成 `do { … } catch { Issue.record("…意外抛错 \(error)") }` —— 不改成 `throws` 用例,
//   为的是让「旧断言文案一条不少」这件事可以被机械核验(逐条 grep 得到)。
...
@Suite("agent 04 任务状态机与工作区 —— AGENTTASK_TESTS passed=(逐条 @Test)")
struct AgentTaskTests {
    ...
    func stateMachine() {
        ...
        #expect(!running.canTransition(to: .pending),
                "任务状态机:running 迁回 pending 非法(时间不倒流)")

        let terminals = AgentTaskState.allCases.filter { $0.isTerminal }
        #expect(terminals.count == 5 && !terminals.contains(.pending) && !terminals.contains(.running),
        ...
        // orphaned 是**猜**出来的终态,必须能被一手证据纠正。
        let orphaned = AgentTaskState.orphaned
        #expect(orphaned.canTransition(to: .completed) && orphaned.canTransition(to: .failed)
        ...
        #expect(!orphaned.canTransition(to: .pending) && !orphaned.canTransition(to: .running)
                && !orphaned.canTransition(to: .orphaned),
                "任务状态机:orphaned 不可复活成 pending/running 也不可迁向自身(纠正不是复活)")
        #expect(!AgentTaskState.completed.canTransition(to: .orphaned)
    ...
    @Test("终态映射与收敛:adapter 终态优先,缺失时按退出码收敛,且绝无挂在 running 的路径")
    func terminalMapping() {
        #expect(AgentTaskState.from(.succeeded) == .completed,
                "终态映射:adapter succeeded 映射为 job completed")
        #expect(AgentTaskState.from(.failed) == .failed,
                "终态映射:adapter failed 映射为 job failed")
        #expect(AgentTaskState.from(.aborted) == .cancelled,
        ...
        #expect(AgentTaskState.resolve(terminal: succeeded, exitCode: 1, cancelRequested: false) == .completed,
                "终态收敛:adapter 给了终态就以它为准(退出码不翻案)")

        // 下面五条全是 Codex 的现实:中断/硬超时时事件流里根本没有终态行,adapter 诚实交回 nil。
        #expect(AgentTaskState.resolve(terminal: nil, exitCode: -15, cancelRequested: false) == .cancelled,
        ...
        #expect(AgentTaskState.resolve(terminal: nil, exitCode: 2, cancelRequested: false) == .failed,
        ...
        #expect(AgentTaskState.resolve(terminal: nil, exitCode: 0, cancelRequested: true) == .cancelled,
                "终态收敛:terminal 缺失但收到过取消意图 判 cancelled")

        // 最要命的一条护栏:无论输入怎么组合,收敛结果必须是终态。
        ...
        #expect(alwaysTerminal,
                "终态收敛:所有输入组合都收敛到终态,绝无把任务挂在 running 的路径")
    }

    // MARK: - ③ task-id 生成

    @Test("task-id:stamp-slug-suffix 形状 + slug 规则(截断 / 中文回退 / 无首尾连字符)")
    func taskID() {
        #expect(AgentTaskID.make(stamp: "20260729-1432", prompt: "Diagnose network. Then report.", suffix: "x7f3")
        ...
        let injected = AgentTaskID.make(stamp: Self.makeClock().now().stamp, prompt: "Diagnose network", suffix: "x7f3")
        #expect(injected == "20260729-1432-diagnose-network-x7f3",
                "task-id:时间前缀经 ClockPort 注入而非读系统时钟(故可逐字断言)")
        #expect(AgentTaskID.slug(from: "Diagnose  NETWORK__x7f3! rest") == "diagnose-network-x7f3",
                "task-id:slug 取首句、小写、连续非字母数字折成单个连字符")

        let long = AgentTaskID.slug(from: "abcdefghij klmnopqrst uvwxyz0123456789")
        #expect(long.count == 24 && !long.hasSuffix("-"),
                "task-id:超长 prompt 的 slug 截到 24 字符以内")
        #expect(AgentTaskID.slug(from: "abcdefghijklmnopqrstuvw xyz") == "abcdefghijklmnopqrstuvw",
                "task-id:截断正好落在连字符上时再去一次尾部连字符")

        #expect(AgentTaskID.slug(from: "诊断一下网络为什么这么慢。谢谢") == "task",
                "task-id:全中文 prompt 折不出字符时回退 task")
        #expect(AgentTaskID.slug(from: "") == "task",
    ...
    @Test("task-id 校验:空串 / 向上穿越 / 含斜杠 / 点开头一律被拒且磁盘零写入,正常 id 照收")
    func taskIDShapeIsValidated() {
        #expect(Self.refusesTaskID(""),
                "task-id 校验:空串被拒且磁盘零写入(空 id 会把工作区根目录本身当成一个任务)")
        #expect(Self.refusesTaskID("../../Users/x/.ssh"),
                "task-id 校验:含两点的向上穿越串被拒且磁盘零写入(生产端口是真 FileManager,会越出 root 写文件)")
        #expect(Self.refusesTaskID("sub/dir"),
                "task-id 校验:含斜杠的 id 被拒且磁盘零写入(目录名即 task-id,只准一层扁平结构)")
        #expect(Self.refusesTaskID(".hidden"),
                "task-id 校验:以点开头的 id 被拒且磁盘零写入(建得出却被 list 与 ls 藏起来的任务是坏证据)")

        // 正常形状的 id 一律照收 —— 校验只拦穿越,不顺手把合法 id 也收窄掉。
        let fs = FakeFileSystem()
        let ws = AgentTaskWorkspace(root: Self.root, fs: fs, clock: Self.makeClock())
        var accepted = false
        do {
    ...
    @Test("工作区:create 的目录结构与 meta 字段(四项 / logs 三件套 / snake_case / 同名不覆盖)")
    func createWorkspace() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect((fs.contents(at: ws.changesPath(for: id)) ?? "").contains("变更清单")
            ...
            #expect(refused, "工作区:同名任务再次 create 抛错,绝不覆盖既有工作区")
        } catch {
            Issue.record("工作区:create 基本路径意外抛错 \(error)")
        }
    }

    @Test("工作区:委托指定外部 workdir 时不建 work 目录,workdir 与 initiator 如实落进 meta")
    func createWorkspaceExternalWorkdir() {
        let (ws2, fs2, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(entries == ["logs", "meta.json", "prompt.md"],
                    "工作区:委托指定外部 workdir 时不建 work 目录")
            #expect(meta.workdir == "/fake/elsewhere/repo" && meta.initiator == "gui",
                    "工作区:外部 workdir 与 initiator 如实落进 meta")
        } catch {
            Issue.record("工作区:外部 workdir 路径意外抛错 \(error)")
        }
    }

    @Test("工作区:缺 meta.json 的半截目录也算已存在,create 抛错且其 logs 证据一字未动")
    func createWorkspaceHalfBuiltDirectory() {
        // 半截目录:上一次「目录与日志都建好了、meta 还没写就崩了」。
        let (ws3, fs3, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(refused && fs3.contents(at: ws3.rawLogPath(for: halfID)) == evidenceBefore,
                    "工作区:缺 meta.json 的半截目录也算已存在,create 抛错且其 logs 证据一字未动")
            #expect(!fs3.exists(at: ws3.metaPath(for: halfID)),
                    "工作区:被拒的 create 没有往半截目录里补写 meta.json(不假装它是自己建的)")
            #expect(try ws3.list().isEmpty,
                    "工作区:缺 meta.json 的半截目录对 list 不可见(已知限制:证据不销毁优先于自动清理)")
        } catch {
            Issue.record("工作区:半截目录用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑤ raw 与 normalized 永不互写

    @Test("工作区:raw.ndjson 里不含归一化消息的任何一行(raw 与 normalized 永不互写)")
    func rawNeverMixesWithNormalized() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(raw.contains(rawA) && raw.contains(rawB),
                    "工作区:两条原始行逐字进了 raw.ndjson(agent 原话全量落盘)")
            #expect(!raw.contains("kind"),
                    "工作区:raw.ndjson 里不含归一化消息的任何一行(raw 与 normalized 永不互写)")
            #expect(normalized.contains("\"kind\"") && normalized.contains("session-started"),
                    "工作区:归一化消息逐行 JSON 进了 normalized.ndjson")
            #expect(!normalized.contains(rawA) && !normalized.contains(rawB),
                    "工作区:normalized.ndjson 里不含任何一条原始行(两个文件内容互不含对方)")
            #expect(normalized.contains("\"callID\" : \"call-1\"") || normalized.contains("\"callID\":\"call-1\""),
                    "工作区:归一化落盘保住 callID(全链不丢)")
            #expect(fs.appendCount(at: ws.rawLogPath(for: id)) == 2
                    && fs.appendCount(at: ws.normalizedLogPath(for: id)) == 3
                    && fs.appendCount(at: ws.stderrLogPath(for: id)) == 1,
                    "工作区:三条追加路径各写各的文件,追加次数逐个对得上")
            #expect(stderr.contains("ERROR some noisy stderr line") && !raw.contains("noisy stderr"),
            ...
            #expect(threw, "工作区:文件系统写入失败时如实抛出,不吞错")
        } catch {
            Issue.record("工作区:raw 与 normalized 分流路径意外抛错 \(error)")
        }
    }

    // MARK: - CR 回填约束:turn 边界不当闸门

    @Test("工作区:turn-started 之前到达的 item 消息照样全量落盘且次序不变(不拿 turn 边界当闸门)")
    func turnBoundaryIsNotAGate() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(itemIndex != nil && turnIndex != nil && itemIndex! < turnIndex!,
                    "工作区:turn-started 之前到达的 item 消息照样全量落盘且次序不变(不拿 turn 边界当闸门)")
            #expect(normalized.split(separator: "\n").count == 3,
                    "工作区:pre-turn 的 item 消息不被丢弃,三条消息全在(丢它就是丢诊断信息)")
        } catch {
            Issue.record("工作区:turn 边界用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑥ session_id 拿到即写

    @Test("工作区:session_id 拿到即经 updateMeta 落盘,且写 meta 绝不顺手碰两个 ndjson")
    func sessionIDWrittenImmediately() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            let metaText = fs.contents(at: ws.metaPath(for: id)) ?? ""
            #expect(meta.sessionID == "sess-abc-123" && metaText.contains("\"session_id\""),
                    "工作区:session_id 拿到即经 updateMeta 落盘(提案 §3 立刻落盘)")
            #expect(meta.state == .running && meta.pid == 4242,
                    "工作区:pending 迁 running 并把 pid 落进 meta(残留判定的唯一依据)")
            #expect(fs.writeCount(at: ws.metaPath(for: id)) == 3,
                    "工作区:meta.json 恰被写了 create 与两次 updateMeta 共三次(单写者)")
            #expect(fs.appendCount(at: ws.rawLogPath(for: id)) == 0
                    && fs.appendCount(at: ws.normalizedLogPath(for: id)) == 0,
                    "工作区:写 meta 绝不顺手碰两个 ndjson(单写者只写自己那份)")
        } catch {
            Issue.record("工作区:session_id 落盘用例意外抛错 \(error)")
        }
    }

    // MARK: - meta 的两个不可变字段(task_id / schema_version)

    @Test("工作区:updateMeta 改 task_id / schema_version 一律被拒,且校验先于写盘")
    func immutableMetaFields() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(fs.contents(at: ws.metaPath(for: id)) == before
                    && fs.writeCount(at: ws.metaPath(for: id)) == writesBefore,
                    "工作区:两次改不可变字段都被拒,meta.json 内容与写入次数皆未变(校验先于写盘)")
        } catch {
            Issue.record("工作区:不可变字段用例意外抛错 \(error)")
        }
    }

    // MARK: - 非法迁移抛错且磁盘一字未改

    @Test("工作区:非法迁移抛错且 meta.json 一字未改;终态元数据冻结、不可复活")
    func illegalTransitionLeavesDiskUntouched() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(threw, "工作区:非法迁移 pending 到 completed 时 updateMeta 抛错")
            #expect(fs.contents(at: ws.metaPath(for: id)) == before,
                    "工作区:非法迁移抛错时 meta.json 内容一字未改(绝不静默改写)")
            #expect(fs.writeCount(at: ws.metaPath(for: id)) == writesBefore,
            ...
            #expect(threwSelf, "工作区:终态任务再迁向自身被拒且 updateMeta 抛错(终态元数据冻结)")

            var threwRevive = false
            do { try ws.updateMeta(taskID: id) { $0.state = .running } } catch { threwRevive = true }
            #expect(threwRevive, "工作区:终态任务迁回 running 被拒且 updateMeta 抛错(终态不可复活)")
            #expect(fs.contents(at: ws.metaPath(for: id)) == frozen,
            ...
            #expect(threwNonTerminal, "工作区:finish 收到非终态时抛错")
        } catch {
            Issue.record("工作区:非法迁移用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑦ 崩溃残留:标 orphaned 且不销毁证据

    @Test("残留扫描:只标 running+pid 已死的那一个,且 logs / prompt / stderr 证据一字未动")
    func orphanScanKeepsEvidence() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            try Self.makeRunningTask(ws, id: dead, pid: 4242)
            ...
            try Self.makeRunningTask(ws, id: live, pid: 100)
            ...
            #expect((try? ws.readMeta(taskID: dead))?.state == .orphaned,
                    "残留扫描:崩溃残留任务的 meta 状态改为 orphaned")
            #expect((try? ws.readMeta(taskID: live))?.state == .running,
                    "残留扫描:pid 仍存活的 running 任务不被误标")
            #expect((try? ws.readMeta(taskID: idle))?.state == .pending,
                    "残留扫描:尚未拉起因而没有 pid 的 pending 任务不被误标")
            #expect((try? ws.readMeta(taskID: noPID))?.state == .running
                    && !liveness.queriedPIDs().isEmpty,
                    "残留扫描:state 为 running 但没记下 pid 的任务不被标 orphaned(没有判据就不判,不凭空断言它死了)")
            #expect((try? ws.readMeta(taskID: dead))?.finishedAt == nil,
                    "残留扫描:不臆造 finished_at 时间戳(并不知道它是何时死的)")

            #expect(fs.contents(at: ws.rawLogPath(for: dead)) == rawBefore
                    && fs.appendCount(at: ws.rawLogPath(for: dead)) == rawWritesBefore,
                    "残留扫描:标 orphaned 后 logs 下的 raw.ndjson 一个字节都没动(证据不销毁)")
            #expect(fs.contents(at: ws.promptPath(for: dead)) == promptBefore
                    && fs.contents(at: ws.stderrLogPath(for: dead)) == stderrBefore,
                    "残留扫描:prompt.md 与 stderr.log 同样一字未动")
            #expect(fs.writeCount(at: ws.promptPath(for: dead)) == 1
                    && fs.writeCount(at: ws.rawLogPath(for: dead)) == 1,
                    "残留扫描:除 meta.json 外没有任何文件被重写")
            #expect(liveness.queriedPIDs().contains(4242) && liveness.queriedPIDs().contains(100),
                    "残留扫描:判定确实走的是按 pid 探活(宿主重启后只剩 pid 可用)")

            // 再扫一遍不该重复标(它已经是终态了)。
            let again = try ws.scanForOrphans(liveness: liveness)
            #expect(again.isEmpty, "残留扫描:已标过的残留不会被重复标记")
        } catch {
            Issue.record("残留扫描用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑦b 孤儿是「猜」出来的:一手证据必须能纠正它

    @Test("孤儿纠正:被扫描抢标 orphaned 后 finish 仍能落定,报告被补出来")
    func orphanRaceIsCorrectableByFinish() {
        let (ws, fs, _) = Self.makeWorkspace()
        let id = "20260729-1432-drain-race-1a2b"
        do {
            try Self.makeRunningTask(ws, id: id, pid: 4242)
            ...
            #expect(marked == [id] && (try? ws.readMeta(taskID: id))?.state == .orphaned,
            ...
            let meta = try ws.readMeta(taskID: id)
            #expect(meta.state == .completed,
                    "孤儿纠正:纠正后 meta 的最终状态是 completed 而不是 orphaned(成功的任务不该被记成孤儿)")
            #expect(meta.exitCode == 0 && meta.finishedAt != nil,
                    "孤儿纠正:纠正的同时把 exit_code 与 finished_at 补齐(扫描当初刻意没填 finished_at)")
            #expect((fs.contents(at: ws.reportPath(for: id)) ?? "").contains("端口 7890 没在听"),
            ...
            Issue.record("孤儿纠正用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑦c finish 同态幂等 + 报告自愈

    @Test("工作区:finish 同态幂等(不重写 meta)+ report.html 缺失时自愈补出")
    func finishIsIdempotentAndSelfHeals() {
        let (ws, fs, clock) = Self.makeWorkspace()
        ...
        do {
            ...
            try ws.finish(taskID: id, state: .completed, exitCode: 0, finalText: "done")
            #expect(clock.callCount() - clockBefore == 1,
            ...
            Issue.record("工作区:finish 幂等用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑦d finish 的 error 与终态一次写盘

    @Test("工作区:finish 的 error 与 state / finished_at / exit_code 一次写盘,成功任务不产 error 噪音键")
    func finishWritesErrorInOneShot() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(meta.error == "model not found: gpt-5-turbo" && meta.state == .failed,
                    "工作区:finish 的 error 参数落进 meta 的 error 字段(失败理由不必再补一次写盘)")
            #expect(fs.writeCount(at: ws.metaPath(for: failed)) == writesBefore + 1,
                    "工作区:error 与 state 与 finished_at 与 exit_code 一次写盘(不留下 error 已填但仍是 running 的半截现场)")
            #expect((fs.contents(at: ws.metaPath(for: failed)) ?? "").contains("\"error\" : \"model not found"),
            ...
            #expect(!(fs.contents(at: ws.metaPath(for: ok)) ?? "").contains("\"error\""),
                    "工作区:成功任务的 error 为 nil 时整键省略,meta.json 不产 error 噪音键")
        } catch {
            Issue.record("工作区:finish 写 error 用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑦e finalText 缺失时退回 normalized 的最后一条 text

    @Test("报告兜底:finalText 为 nil 时退回 normalized 里最后一条 text 消息(Codex 侧的唯一来源)")
    func finalTextFallsBackToLastNormalizedText() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(silentHTML.contains("没有留下最终文本") && silentHTML.contains("logs/raw.ndjson"),
            ...
            #expect(!threw && (fs3.contents(at: ws3.reportPath(for: broken)) ?? "").contains("没有留下最终文本"),
                    "报告兜底:normalized.ndjson 读不出来时退回 nil 而不是抛错(兜底是尽力而为,不掀翻已写定的终态)")
        } catch {
            Issue.record("报告兜底:最终文本退回路径意外抛错 \(error)")
        }
    }

    // MARK: - ⑧ HTML 报告

    @Test("HTML 报告:escape 顺序不可颠倒(先 amp 再其余),五个危险字符逐个转义")
    func reportEscaping() {
        ...
        #expect(AgentTaskReport.escapeHTML("&<>\"'") == "&amp;&lt;&gt;&quot;&#39;",
                "HTML 报告:五个危险字符逐个转义为 amp/lt/gt/quot/#39")
    }

    @Test("HTML 报告:缺 report.html 时兜底生成(自包含零外链),终态字段同时落进 meta")
    func reportFallbackGeneration() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(html.contains(AgentTaskReport.fallbackMarker),
                    "HTML 报告:缺 report.html 时兜底生成,且页脚显式标注由文本兜底生成")
            #expect(html.contains("a &amp; b &lt; c"),
                    "HTML 报告:兜底页正文是 escape 后的最终文本")
            #expect(!html.contains("http://") && !html.contains("https://") && !html.contains("<script"),
                    "HTML 报告:兜底页自包含,零外链零脚本")
            let meta = try ws.readMeta(taskID: fallbackID)
            #expect(meta.state == .completed && meta.exitCode == 0 && meta.finishedAt != nil,
                    "HTML 报告:finish 同时把终态与 exit_code 与 finished_at 落进 meta")
        } catch {
            Issue.record("HTML 报告:兜底路径意外抛错 \(error)")
        }
    }

    @Test("HTML 报告:已有 agent 自产的 report.html 时原样保留,绝不覆盖")
    func reportAgentAuthoredIsPreserved() {
        let (ws2, fs2, _) = Self.makeWorkspace()
        ...
        do {
            ...
            #expect(fs2.contents(at: ws2.reportPath(for: agentID)) == agentHTML,
                    "HTML 报告:已有 agent 自产的 report.html 时原样保留,绝不覆盖")
            #expect(fs2.writeCount(at: ws2.reportPath(for: agentID)) == 1,
                    "HTML 报告:agent 自产报告只被写过一次(兜底没有再写一遍)")
            #expect(!(fs2.contents(at: ws2.reportPath(for: agentID)) ?? "").contains(AgentTaskReport.fallbackMarker),
                    "HTML 报告:agent 自产报告里不会被塞进兜底标注")
        } catch {
            Issue.record("HTML 报告:自产报告路径意外抛错 \(error)")
        }
    }

    @Test("HTML 报告:没有最终文本时如实说明并指向 raw.ndjson,不硬造内容")
    func reportEmptyFinalText() {
        let empty = AgentTaskReport.fallbackHTML(taskID: "t", state: .failed, finalText: nil, generatedAt: "2026-07-29T15:00:00Z")
        #expect(empty.contains("logs/raw.ndjson") && empty.contains(AgentTaskReport.fallbackMarker),
                "HTML 报告:没有最终文本时如实说明并指向 raw.ndjson,不硬造内容")
    }

    // MARK: - ⑨ prune 永不删 running / pending

    @Test("prune:running 与 pending 哪怕被点名要删也跳过,终态任务整棵目录树递归删除")
    func pruneNeverDeletesLiveTasks() {
        let (ws, fs, _) = Self.makeWorkspace()
        ...
        do {
            ...
            try Self.makeRunningTask(ws, id: running, pid: 555)
            ...
            #expect(listedBefore == [done, kept, running, pending].sorted(),
                    "prune:list 只把含 meta.json 的条目当任务,四个任务全在")

            // 点名保留 kept;其余三个都是「点名要删」——但 running/pending 必须被跳过。
            let result = try ws.prune(keepIDs: [kept])
            #expect(result.removed == [done],
                    "prune:终态任务目录被删除")
            #expect(result.skipped == [running, pending].sorted(),
                    "prune:running 与 pending 哪怕被点名要删也跳过,并如实出现在 skipped")
            let listedAfter = try ws.list()
            #expect(listedAfter == [kept, running, pending].sorted(),
                    "prune:删除后 list 里不再有该任务,其余三个原封不动")
            #expect(fs.contents(at: ws.rawLogPath(for: done)) == nil
                    && fs.contents(at: ws.metaPath(for: done)) == nil,
                    "prune:被删任务的整棵目录树都清掉了(递归删除)")
            #expect(fs.contents(at: ws.rawLogPath(for: running)) != nil,
                    "prune:被跳过的 running 任务的日志证据一字未动")
            #expect(fs.contents(at: ws.metaPath(for: kept)) != nil,
                    "prune:keepIDs 点名保留的终态任务不删")
        } catch {
            Issue.record("prune 用例意外抛错 \(error)")
        }
    }

    // MARK: - ⑩ 读侧容忍未知字段 + 写侧原样写回

    @Test("读侧容忍未知字段 + 写侧原样写回(旧版本不得静默剥掉新版本的字段)")
    func unknownFieldTolerance() {
        let fs = FakeFileSystem()
        let ws = AgentTaskWorkspace(root: Self.root, fs: fs, clock: Self.makeClock())
