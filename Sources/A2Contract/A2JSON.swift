...
///
/// **已知边界:超出 `Int` 范围的整数会静默落到 `.double`**(解码按 Int → Double 的顺序试)。
/// 后果是精度丢失 + 重编码时形状变成浮点。为什么现在不管:
///   * 契约里所有整数字段都过 zod 的 `.int()`,而 zod 的整数上限是 `Number.MAX_SAFE_INTEGER`
///     (2^53-1)—— 那**远小于** `Int64` 上限,所以合法报文里根本产生不出会溢出的整数;
///   * 能力自定义的 `output`/`input` 是任意 JSON,理论上塞得进一个 20 位整数(比如某个外部 API 的 id)。
///     真出现时表现是"往返后 1e19 变成 1e+19",金标往返断言会红 —— 不会静默错到线上。
/// 真要支持,该加的是 `.uint64` / 十进制字符串保真分支,而不是把整数一律当 Double。
