...
//   * `PATCH /configs` —— 改运行参数(mode)。技术上对别人那份也成立:它改的是对方自己那份配置里的
//     一个开关,不换文件、不动进程。
...
//
// **写面当前整体停用**(2026-08-12 用户裁定:restful 控制暂时关掉,读状态就够了)。下面三个写函数
// 与它们的测试原地留着,但**没有开放的调用方** —— 唯一的调用点在 `capability/proxy.ts` 那几条
// 已摘掉注册的能力里(见 `DISABLED_CAPABILITY_IDS`)。恢复能力注册即恢复,不需要动本文件。
