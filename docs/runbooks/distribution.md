...
| `A2 Panel.app` | 可选随附的菜单栏壳（zip） | mac 用户，且想要 dangerous 确认弹窗 |
...
两条 fail-closed 的结构约束写在 schema 里（`ReleaseManifestSchema`），不是写在注释里：
**声明文本与 GPL 全文各恰好一份**、**不认识的文件不许混进发布包**。任一条不满足，元数据就生成不出来。
...
a2 about            # 读许可与外部程序声明
a2 service install  # 装成系统托管的常驻服务
```
...
拿 `.app` 的话另外删 `/Applications/A2 Panel.app`（它不写除 TCC 授权之外的任何系统状态）。
