...
> ⚠️ **2026-08-05（内核 bin 化 10 票「壳原子切换」）后的口径变更 —— 先读这条**：
> 本文写于旧拓扑(`AA.app` = `aahost` + `aa` + **随包 GPL mihomo 二进制** + 内核重签)。
> 那三样东西已在 10 票退场：
>
> | 本文写的 | 现在的事实 |
> |---|---|
> | 产物 `AA.app`，bundle id `com.aa.host` | 产物 **`A2 Panel.app`**，bundle id **`com.a2.panel`**（aa 系命名全面退场） |
> | 主可执行 `aahost`，包内另有 `aa` 与 mihomo | 包里**只有一个 Mach-O**：`Contents/MacOS/a2-panel`（`Scripts/build-app.sh` 有结构红线断言盯着） |
> | 「内核重签入构建链」（逐个 `--identifier`） | **整条废除**：不再分发 GPL 二进制（ADR 0007 修订版），没有内嵌代码要先签 |
> | `aa install-cli` 把 CLI 链接进 PATH | CLI 由内核 bin `a2` 自己提供，走单文件下载 + curl 安装脚本 |
>
> **仍然有效的部分**：§0（ad-hoc 是 Phase 1 终态）、证书申请与 `security find-identity` 的操作、
> TCC / 通知授权的机制说明、公证前置条件。授权仪式要做就**对着 `com.a2.panel` 做**
> （旧 id 从未上过真证书、也从未授过权，所以换 id 的代价此刻恰好是零）。
> 全文按新形态重写归 **13 票**（分发工件），本次只加这段前言、正文一字未改 —— 免得留一份
> 「看起来可以照做、照做却全是旧路径」的运维文档。
>
