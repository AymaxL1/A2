...
3. **执行器白名单**：面板经内嵌 bin 执行的命令**只有四条**——`service install`（带 `--copy-to-home`）、`service uninstall`、`service status`、`version`，全部走机读 JSON 输出。白名单是硬的：壳里没有第二条通往内核 bin 的路，也没有「随便执行一条 a2 命令」的口子。壳仍**不含业务逻辑**（[ADR 0008](0008-kernel-bin-ui-optional.md) 第 5 条结构红线不破）：它只发起命令、解析机读结果、呈现。
   ...
   > **修订（2026-09-04，url-router 05 票）**：白名单**恰增两条**，共十二条——`url-router status --json`（只读）与 `url-router restore --json`。它们只服务于卸载序列的前置（第 6 条修订）：先问「A2 Panel 还是不是默认浏览器」，是就先还原。**`url-router takeover` 不在白名单里**——接管是用户主动发起的事，卸载路径不该拥有它。原则不变：每条占一个枚举成员、逐字进断言、只读机读 JSON、壳不含业务逻辑（该不该 restore 由内核算好的 `handler.matchesTarget` 说了算）。一条代价如实记：`restore` 是白名单里**唯一会等人的命令**（系统弹框，内核那侧的窗 120s），等待期间引导面锁在「卸载中…」。<br>**为什么走内嵌 bin 而不是既有的 UDS 会话**：壳自己就是这条命令的机械执行器（[ADR 0015](0015-url-router-default-browser.md) 的执行链），若由会话线程自己发起，那帧回推的执行指令会被缓冲进推送队列而永不派发——内核等满 120s、壳等到连接报废，一次互等的死锁。走内嵌 bin 时发起方是另一个进程，会话线程照常空闲收推送。

   > **修订（2026-09-04，url-router 06 票；[spec §15 修正案第 10 条](../../.scratch/url-router/spec.md)）**：白名单**恰增一条**，共十三条——`url-router takeover --json`。上一条修订里「takeover 不在白名单里」的判断**就此作废**：那时白名单只服务于卸载序列，而本票按 spec §9 补上了**面板的接管入口**（菜单项「设为默认浏览器…」），发起方仍是用户的一次显式点击，只是入口从 CLI 多了一个。它与 restore **同族同纪律**：同一条内嵌 bin 通道（同一个死锁理由，见上条）、同样会等人（系统为 http/https 各弹一次框，内核那侧的窗 120s，等待期间引导面锁住）、失败原样转达（内核的 `code` + `guidance` 逐条进菜单）。两条边界如实写明：①**壳侧不加确认框**——系统弹框就是这条 dangerous 命令的确认器（[ADR 0015](0015-url-router-default-browser.md) 可复用确认器原则），壳再问一遍就是双确认；②**壳不做「已是默认就隐藏」的状态判断**——那要多存一份下一秒就会过期的系统状态副本，而 takeover 幂等（已是默认时内核答 `already: true`，一个系统调用都不发、一个框都不弹），于是菜单项恒常在场、不随状态变形。
...
6. **卸载对等**：能装就能卸。「停止并卸载内核服务」走同一条白名单（`service uninstall`，带确认弹窗），**只拆 unit**；`$A2_HOME/bin/a2` 那份拷贝与 `~/.a2` 里的数据留给显式清理——与 `install.sh --uninstall` 的「先看后删」是同一种姿势（数据同侧的东西不由一次点击带走）。**删掉 `.app` 不会卸掉已装的服务**，这正是第 4 条拷贝的直接后果，必须在 runbook 里说明。
   ...
   > **修订（2026-09-04，url-router 05 票）**：卸载序列**多一个打头的前置**——A2 Panel 若仍是系统默认浏览器，先 `url-router restore`（系统弹框，http/https 各一次），**用户在那两个框上点取消/超时即中止本次卸载**并原样呈现内核的指引，一条卸载命令都不发。理由与上面②同构：还原默认浏览器的唯一入口（`a2 url-router restore`）住在 `$A2_HOME/bin/a2` 里，卸掉之后点任何链接都会去拉一个不在的 app，而收拾它的命令也没了。确认框正文事先说清这两个框会出现（否则它们像是凭空冒出来的）。同一条判据在**内核侧也有一份**（`service uninstall --purge` 的 `service_purge_url_handler_taken`，退出码 1）：面板路径与 CLI 野路径是两个真实入口，各兜一半。两侧都只拦「**确知**还挂着」——读不出系统 handler 时放行，否则 Linux 与没装过 Panel 的机器会永远卸不掉。
