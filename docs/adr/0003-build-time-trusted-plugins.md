...
上一条 Consequences 预留的路径已被行使：[ADR 0011](0011-plugin-exec-protocol-loading.md)（插件 exec 协议与装载）**取代本 ADR 的范围部分**——

- **被取代**：「插件是 monorepo 内构建时集成的包、不存在运行时装载面」→ 现有运行时装载面 `a2 plugin add`（本机用户/agent 自己写的单文件 `.ts`）；「强逻辑隔离、不做一插件一进程」→ 现在就是一插件一进程（进程外子进程，一次调用一次 exec）。
- **继续有效**：插件市场、面向不受信任第三方作者的分发与审核继续暂缓；manifest / 能力契约要素继续由 [ADR 0004](0004-capability-registry-sole-call-surface.md) 承载。
