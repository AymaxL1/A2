...
**状态（2026-08-04，16 票脚本部分）：Phase 1 出口尚未达成。** 只回写「脚本部分」的结论，两条必须有人在场的实测**未做**：

- ✅ **已做（自动化）**：`Scripts/check/flagship-e2e.sh`（断言组 FS，5 条）用**一个宿主实例**、**全程只经 `aa`** 走通旗舰链
  开代理 → 切模式 → 选节点 → 更新已有订阅，逐步骤断言退出码与读回；「零 GUI 打断」以三条证据合证
  （宿主处于会弹窗的 `interactive` 确认档 + 全窗口无任何 `[confirm]` 日志行 + 每步在墙钟时限内返回且无一返回 `pending`）；
  并有**反向对照**：同一路由下 dangerous 换源（`proxy.subscription.add`）确实触发确认、`AA_CONFIRM_AUTO=deny` 挡下（退出码 2）。
  门禁 `bash Scripts/check.sh` = PASS 428 / FAIL 0。**注意：这里的「agent」是 shell 脚本模拟的，不是真 Codex。**
- ❌ **未做（必须有人在场）**：真 Codex 按 `aa docs agents-md` 接入、`prefix_rule` 一次批准后在沙箱外完成旗舰操作的实测记录。
  拉起真 codex 会消耗用户真实配额且需要交互式批准，无法在无人值守门禁里做。
- ❌ **未做（必须有人在场）**：换源场景的真机点验 —— Codex 发起 → 宿主弹**真 NSAlert** → 批准/拒绝两分支的行为与记录。
  门禁里验的是 `AA_CONFIRM_AUTO` 这条 test-only seam 短路后的等价路径，**没有任何一次真弹窗被人看见过**。

**故 Phase 1 出口判据（旗舰场景通过）目前不成立**：脚本证明了这条链在无人值守下是通的，
但验收辞里的「Codex」与「宿主确认弹窗」这两件事都还没有真机证据。补齐上述两条人工项后方可判 Phase 1 出口达成。
