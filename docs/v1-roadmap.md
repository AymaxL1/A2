...
| 1 | 平台最小核 + **代理插件** → **（2026-08-04 改写）内核 bin 化重构** | **重构蓝图第⑤步（壳原子切换）完成**：`a2` 单 bin + `a2 service` 安装 + mihomo 监督 + 三层仲裁与确认器 + 壳原子切换 + 门禁原子切到 TS 四件套 |
...
6. 门禁在此步**原子切换**到 TS 门禁四件套，`check.sh` 退役。
...
| ⑤ | **壳原子切换**：`a2-panel` 接新内核、旧可执行废除、快照测试跟切 | **唯一门禁切换点**：`check.sh` → TS 门禁四件套 |
...
- **⑤之时**：原子切换到 **TS 门禁四件套** —— ① `bun test`（CLI 面为主战场：argv 进、stdout JSON / 退出码出）；② **契约金标报文快照**（TS 与 Swift 双端对同一批样本编解码，防契约漂移）；③ **壳快照测试**（既有「一个模型两个渲染器」手搓快照原封保留，继续当壳侧回归门禁）；④ **重写版 flagship e2e**（对真实 `a2` bin + 假 mihomo 夹具跑端到端）。
- **断言迁移**：既有 428 断言按**行为对等**逐条映射到 TS 侧，允许合并/淘汰只属 Swift 实现细节的断言；Swift 逻辑代码同期作行为规范参考。
- **⑤之后**：`check.sh` 退役。
...
- 合入后，这批工作的价值**转为行为规范参考**：代理域的能力面语义、订阅/模式/节点行为、以及 17 票迁到 `Tests/` 的全套 swift-testing 断言（428 断言口径），是 TS 重建时的行为规格来源，不再是主干实现。
...
| # | 原形态 | ⑤后的新形态 |
|---|---|---|
| 1 | 装 Xcode + 签发 Apple Development 证书 | 不变（前置条件），但签的对象是 **`a2-panel.app`**；内核 bin `a2` 走单文件下载分发，不吃 .app 签名链 |
| 2 | 用真证书重签 `.app` + 重跑门禁 | 对 `a2-panel.app` 出包 + 跑 TS 门禁（壳快照 + e2e） |
| 3 | 首次 TCC / 通知授权点头 | 归**确认器**：`a2-panel` 以确认器角色呈现 Touch ID / 通知，授权仪式在它身上做一次 |
| 4 | 真 Codex 经 `aa docs agents-md` + `prefix_rule` 完成旗舰操作 | 对 **`a2 … --json`** 重跑同一场景 |
| 5 | 换源 dangerous 真机点验（真弹窗批准/拒绝两分支） | 对**确认器**重跑：带外确认的批准/拒绝两分支，**外加一条无确认器时的默拒实测**（新架构的第①层，Linux 端的默认路径） |
